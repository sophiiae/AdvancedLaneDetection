...
def slidewins(binary, nwindows=14, margin=200, minpix=100):
...
def fit_poly(binary):
    ...
    leftx, lefty, rightx, righty, out_img = slidewins(binary)
...
def search_poly(binary, margin=200):
    # Fit new polynomials
    left_fitx, right_fitx, ploty, out_img = fit_poly(binary)
...
def check_centroids(warped, window_centroids, window_width, window_height, margin):
...
def find_centroids(image, window_width, window_height, margin):
    window_centroids = [] 
    window = np.ones(window_width) # Create our window template
    ...
    for level in range(1,(int)(image.shape[0]/window_height)):
	    ...
	    window_centroids.append((l_center,r_center))
    out = check_centroids(image, window_centroids,window_width, window_height, margin)
    return out
